---
title: On Pomodoro
excerpt: How to create a minimal Pomodoro menu bar app with Keyboard Maestro.
created: '2021-11-07T00:00:00.000Z'
updated: '2021-11-07T00:00:00.000Z'
---
...
The rest of the post will go into detail about the steps and the logic behind it, but the complete macro group can be downloaded from the [conclusion](#conclusion).

## Table of Contents

## Background

The [Pomodoro technique](https://en.wikipedia.org/wiki/Pomodoro_Technique) is a time management method that has you alternate between periods of focus and short breaks. Normally it's a 25-minute period of work followed by 5 minutes of break, which can then be repeated multiple times.
...
1. At rest, there's an item in the menu bar to open up a menu for starting a Pomodoro.

    ![Screenshot of "Keyboard Maestro → Pomodoro → Start"](/posts/on-pomodoro/pomodoro-menu-start-light.png)

2. Clicking the menu item lets me configure the next Pomodoro run: how many loops there should be, and how long each Pomodoro should last. This defaults to 4 loops of 25 minutes, meaning that it covers a period of 2 hours.

    ![Screenshot of "Keyboard Maestro → Pomodoro → Prompt"](/posts/on-pomodoro/pomodoro-prompt-light.png)

3. Starting a Pomodoro will show the remaining time in the menu bar.

    ![Screenshot of "Keyboard Maestro → Pomodoro → Stop"](/posts/on-pomodoro/pomodoro-menu-stop-light.png)

4. In between Pomodoros, the menu bar will show that you're on a break.

    ![Screenshot of "Keyboard Maestro → Pomodoro → Break"](/posts/on-pomodoro/pomodoro-menu-break-light.png)

5. After the break interval, steps 3–4 are repeated, until there are no more loops remaining.

Since this is all done with Keyboard Maestro I can easily hook in any of my other macros. For example, when a Pomodoro starts it [turns on Do Not Disturb](/posts/on-desktop-productivity#toggling-do-not-disturb) and turns my desk light red, and at the end of a Pomodoro it resets both. This part is really up to the user, but having it together with my other macros like this offers a lot of flexibility.

## Macros

The macro group contains four individual macros, and is set to display in the menu bar with a title from the variable `POMODORO_TITLE`. It has no icon, since I prefer to set that through text as well to have them display nicely in the same color.

_Note: For custom icons you can download Apple's [SF Symbols](https://developer.apple.com/sf-symbols/) for free, which is the same set of icons that have just been added to Keyboard Maestro itself._
...
In the second "Logic" group I use the configured duration to set the time remaining (`POMODORO_REMAINING`). This is used to keep track of the Pomodoro progress. I then enable the "Stop" macro and disable the "Start" macro, which ensures that only one of them will ever be visible in the menu bar menu: "Start" when no Pomodoro is running, and "Stop" otherwise (see also my approach to [starting and stopping modes](/posts/on-desktop-productivity#starting-and-stopping-modes)). Finally, I execute the "Update" macro.
...
Once the minute has passed, I decrease the `POMODORO_REMAINING` by 1. If the result is 0 the run the "[stop macro] [stopped](#stop)" is triggered, and otherwise it goes back to the beginning of the repeating group.

![Screenshot of "Keyboard Maestro → Pomodoro → Update"](/posts/on-pomodoro/update-light.png)

### Stop

Similarly to the ["Start"](#start) macro, this contains a "Logic" group and a few actions outside of the group. Those actions outside are again up to the user to allow for customization.

In the "Logic" group, first the earlier saved macro instance is canceled. As described, this is in case the Pomodoro is stopped prematurely through the menu bar item. Important to note here is that this is also the reason the "Execute Macro" action in ["Update"](#update) was called asynchronously; if not, "Stop" would be seen as a child of the "Update" instance, meaning it'd cancel itself.
...
If, instead, there are still more loops to run, it will [set the title](#set-title), pause for 5 minutes, and then execute the "Start" macro again. This is done asynchronously for the same reason as described earlier.

![Screenshot of "Keyboard Maestro → Pomodoro → Stop"](/posts/on-pomodoro/stop-light.png)

### Set Title

The "Set Title" macro is set up as a subroutine. It accepts a parameter `Local State`, and then uses a switch action to update the `POMODORO_TITLE` variable in one of three ways:
